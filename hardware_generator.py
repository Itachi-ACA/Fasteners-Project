...
# ============================================================
# BUILT-IN WEIGHT TABLE — fallback when no Excel is uploaded
# OR when a specific size is missing from the uploaded table.
# Contains HDG pcs/kg values only. SS/MAGNI/HT show N/A because
# those coatings have no weight data until user uploads their sheet.
# ============================================================
...
def _parse_hardware(lines):
    ...
    for line in lines:
        u = line.upper()
        # Must contain a valid bolt/stud size pattern
        if not re.search(r"[Ss]?-?M\d+X\d+", u):
            continue
        # Skip header / label rows that have no numbers
        if not re.search(r"\d{1,4}\s+\d{1,4}", line):
            continue

        parts = line.split()
        for i, part in enumerate(parts):
            if not re.match(r"^(S-)?M\d+X\d+-?[A-Z]*$", part.upper()):
                continue
            try:
                qty    = int(parts[i+1])
                nut    = int(parts[i+2])
                l_nut  = int(parts[i+3])
                p_wash = int(parts[i+4])
                s_wash = int(parts[i+5])
            except (IndexError, ValueError):
                continue

            # Determine fastener type by S- prefix — most reliable signal
            if part.upper().startswith("S-"):
                fastener = "STUD"
            else:
                # Check nearby tokens for STUD keyword (fallback)
                context = " ".join(parts[max(0,i-3):i]).upper()
                fastener = "STUD" if "STUD" in context else "BOLT"
...
def _is_m6_or_below(size_material):
    """M6 and below (M4, M5, M6) → always SS coating in output."""
    key = re.sub(r"^S-","", size_material.upper().strip())
    m = re.search(r"M(\d+)", key)
    return int(m.group(1)) <= 6 if m else False

def _round_size_material(size_material):
    """Round length up to nearest 10. Force SS coating for M6 and below (M4, M5, M6).
    M8 and above keep their original coating from input sheet."""
    ...
    if m:
        ...
        coating  = "-SS" if size_num <= 6 else m.group(3)
...
def _get_weight(size_material, qty):
    ...
    m = re.match(r"(M\d+)X(\d+)(-[A-Z]+)", key)
    if m:
        diam    = int(re.search(r"\d+", m.group(1)).group())
        rounded = math.ceil(int(m.group(2))/10)*10
        coating = "-SS" if diam <= 6 else m.group(3)
        key = f"{m.group(1)}X{rounded}{coating}"
...
def run_export(pdf_path, out_folder, open_after, progress_cb, done_cb, error_cb):
    ...
    try:
        ...
        for sm, qty in sorted(bolt_grouped.items()):
            pcs, wt = ("N/A","N/A") if _is_m6_or_below(sm) else _get_weight(sm, qty)
...
def show_splash():
    ...
    for i in range(11):
        splash.attributes("-alpha",i*0.1); splash.update(); time.sleep(0.02)
    try:
        import winsound
    ...
    for prog, msg in [(0.2,"LOADING WEIGHT TABLE ENGINE..."),(0.4,"READING HARDWARE DATA..."),
        ...
        cv.itemconfig(st, text=msg); splash.update(); time.sleep(0.12)
...
def launch_main():
    ...
    nav_help, click_help = make_nav_item("⬡","Help / README", "help")
    ...
    # PAGE: HELP / README
    # ============================================================
    help_frame = tk.Frame(content_area, bg=BG_DEEP())
    pages["help"] = (help_frame, nav_help)
    make_topbar(help_frame, "HELP  /  USER GUIDE")
    tk.Frame(help_frame, bg=BORDER(), height=1).pack(fill="x")

    help_body = tk.Frame(help_frame, bg=BG_DEEP())
    help_body.pack(fill="both", expand=True)
    help_scroll = tk.Scrollbar(help_body)
    help_scroll.pack(side="right", fill="y")
    help_text = tk.Text(
        help_body, bg=BG_DEEP(), fg=FG(), font=("Consolas",10),
        relief="flat", wrap="word", padx=28, pady=20,
        yscrollcommand=help_scroll.set, state="normal",
        cursor="arrow", spacing1=2, spacing3=4,
    )
    help_text.pack(fill="both", expand=True)
    help_scroll.config(command=help_text.yview)

    help_text.tag_config("h1",  font=("Consolas",16,"bold"), foreground=ACCENT,   spacing1=14, spacing3=6)
    help_text.tag_config("h2",  font=("Consolas",12,"bold"), foreground=BLUE,     spacing1=12, spacing3=4)
    help_text.tag_config("h3",  font=("Consolas",10,"bold"), foreground=YELLOW,   spacing1=8,  spacing3=2)
    help_text.tag_config("body",font=("Consolas",10),        foreground=FG())
    help_text.tag_config("note",font=("Consolas",9),         foreground=FG2())
    help_text.tag_config("ok",  font=("Consolas",10),        foreground=GREEN)
    help_text.tag_config("warn",font=("Consolas",10),        foreground=YELLOW)
    help_text.tag_config("err", font=("Consolas",10),        foreground=RED)
    help_text.tag_config("div", font=("Consolas",10),        foreground=FG_MUTED())

    def hline(): help_text.insert("end","─"*82+"\n","div")
    def h1(t):   help_text.insert("end",f"\n  {t}\n","h1")
    def h2(t):   help_text.insert("end",f"\n  ▌ {t}\n","h2")
    def h3(t):   help_text.insert("end",f"    ► {t}\n","h3")
    def p(t):    help_text.insert("end",f"    {t}\n","body")
    def note(t): help_text.insert("end",f"    ℹ  {t}\n","note")
    def ok(t):   help_text.insert("end",f"    ✔  {t}\n","ok")
    def warn(t): help_text.insert("end",f"    ⚠  {t}\n","warn")
    def err(t):  help_text.insert("end",f"    ✖  {t}\n","err")
    def blank(): help_text.insert("end","\n")

    h1("INDOTECH HARDWARE AUTOMATION PLATFORM  —  USER GUIDE  v1.3")
    hline()
    p("This application reads a Hardware List PDF prepared by the IT team,")
    p("automatically counts all bolts, studs, nuts and washers, calculates their")
    p("total weight in KG, and saves a clean Excel + PDF output report.")
    p("No manual counting. No spreadsheet formulas. One click.")
    blank()

    h2("WHAT THIS APP DOES — IN SIMPLE WORDS")
    hline()
    p("The Design team gives you a Hardware List PDF for each work order.")
    p("That PDF has many rows — bolt sizes, quantities, nuts, washers.")
    p("This app reads that PDF, groups all sizes together, calculates weight,")
    p("and saves a final Excel + PDF file for the Production / Purchase team.")
    blank()
    p("  INPUT   →   Hardware List PDF  (from Design team)")
    p("  OUTPUT  →   Hardware Output Excel + PDF  (for Production / Purchase)")
    blank()

    h2("FREQUENTLY ASKED QUESTIONS")
    hline()
    h3("Do I need to upload the weight table every time I open the app?")
    ok("No. Upload once, click 'Save as Default', and it loads automatically forever.")
    blank()
    h3("Can I process multiple PDFs one after another?")
    ok("Yes. After one is done, just drop a new PDF and click Generate again.")
    blank()
    h3("What if the same bolt size appears many times in the PDF?")
    ok("The app adds all quantities together. You get one combined row per size.")
    blank()
    h3("The PCS/KG value in output is different from my weight chart. Why?")
    p("The app first checks the uploaded Excel. If a size is missing there,")
    p("it uses the built-in HDG table as a backup.")
    p("Ensure your Excel has all required sizes to avoid this.")
    blank()
    h3("What does the Activity Log show?")
    p("Every step the app took, with time. If anything fails, the exact")
    p("error message appears there in red — share this with the developer.")
    blank()

    
    h2("HOW TO USE — STEP BY STEP")
    hline()
    h3("STEP 1 — Open the App")
    p("Double-click the Python file or run it from the terminal.")
    p("A splash screen appears briefly, then the main window opens.")
    blank()
    h3("STEP 2 — Load the Weight Table  (first time only)")
    p("Click 'Weight Table' in the left menu.")
    p("Click 'Upload Weight Table Excel' and select your Hardware_list.xlsx file.")
    p("When asked 'Save as Default?' — click YES.")
    p("From next time, it loads by itself. You never need to do this again")
    p("unless the weight chart changes.")
    blank()
    h3("STEP 3 — Select the Hardware PDF")
    p("On the Dashboard, you will see a dotted box saying 'DROP PDF HERE'.")
    p("You can either:")
    p("  a)  Drag and drop the PDF file directly onto that box,  OR")
    p("  b)  Click the '⊕ BROWSE FOR PDF' button and select the file.")
    p("Once selected, the box turns green and shows the filename.")
    blank()
    h3("STEP 4 — Generate the Output")
    p("Click the orange 'GENERATE HARDWARE LIST' button.")
    p("A folder selection dialog opens — choose where to save the output.")
    p("A progress bar shows each processing step.")
    p("A popup appears when done, showing the result summary.")
    blank()
    h3("STEP 5 — Find Your Output Files")
    p("A new folder is created inside the location you chose:")
    p("  {WorkOrder}_Hardware_Output\\")
    p("    ├── Hardware_Output_{WorkOrder}.xlsx   ← Excel file")
    p("    └── Hardware_Output_{WorkOrder}.pdf    ← PDF file")
    p("The PDF opens automatically after generation (if that option is ticked).")
    blank()

    h2("SIDEBAR MENU — WHAT EACH SECTION DOES")
    hline()
    h3("Dashboard")
    p("Your main working screen. Drop PDF here, click Generate.")
    p("The 4 cards at the top update after each generation:")
    p("  Bolt Types   = number of different bolt sizes found in the PDF")
    p("  Stud Types   = number of different stud sizes found in the PDF")
    p("  Total Weight = combined hardware weight in KG")
    p("  Work Order   = work order number extracted from the PDF")
    blank()
    h3("Weight Table")
    p("Upload your standard weight chart Excel file here.")
    p("The Excel must have one sheet per coating (HDG, SS, HT, MAGNI, etc.).")
    p("Each sheet must have columns: Description, Unit, Qty Per Kg.")
    p("  Unit = Kg  →  weight is calculated from pcs/kg value.")
    p("  Unit = No  →  weight shows N/A (no data available).")
    blank()
    h3("History")
    p("Shows all work orders processed in this session.")
    p("Click '📂 Open' to open the output folder for any previous generation.")
    blank()
    h3("Activity Log")
    p("Full record of every action with timestamps.")
    p("If something goes wrong, this is the first place to check.")
    p("Click CLEAR LOG to reset it.")
    blank()
    h3("Settings")
    p("Change the names that print at the bottom of the output sheet:")
    p("  Prepared By, Checked By, Approved By.")
    p("Change the Format Number (default: F/DSN/54).")
    p("Set a remembered output folder — skips the browse dialog next time.")
    blank()

    h2("COATING AND ROUNDING RULES  (AUTOMATIC)")
    hline()
    p("These rules run automatically. You do not need to do anything:")
    blank()
    ok("M6 and below  (M4, M5, M6)   →  coating always shown as SS in output")
    ok("M8 and above                  →  coating taken as-is from the input PDF")
    ok("Lengths ending in 5 (e.g. M12x35) →  rounded UP to next 10 (→ M12X40)")
    ok("Sizes starting with S-M...    →  always treated as STUD")
    ok("L.STUD rows                   →  treated same as F.STUD (both = stud)")
    ok("Rows with blank Fastener col  →  still captured based on size pattern")
    blank()

    h2("IF SOMETHING GOES WRONG — TROUBLESHOOTING")
    hline()
    h3("PDF selected but Generate does nothing or shows error")
    err("Make sure the PDF is NOT a scanned image (photo of paper).")
    err("The PDF must be a real digital PDF created from software.")
    note("Check Activity Log for the exact error message.")
    blank()
    h3("Excel saved but PDF was not created")
    err("Microsoft Excel must be installed on this computer.")
    err("Close Excel if it is already open with the same file.")
    note("The Excel file is always saved even if PDF export fails.")
    blank()
    h3("Total Weight shows N/A for some sizes")
    warn("SS / MAGNI / HT sizes show N/A until you upload their weight rates.")
    warn("HDG sizes use the built-in table if the uploaded Excel does not have them.")
    blank()
    h3("Weight Table not loading at startup")
    warn("Go to Weight Table page → Upload the Excel → click 'Save as Default'.")
    blank()
    h3("Error says 'No BOLT/STUD lines found in PDF'")
    err("The PDF format is different from the standard IndoTech Hardware List.")
    err("Check that the PDF has FASTENER and SIZE-MATERIAL columns.")
    blank()
    h3("App crashes or freezes")
    err("Close the app. Make sure no Excel file from this work order is open.")
    err("Restart the app and try again.")
    blank()
    h3("Logo not showing in the Excel/PDF output")
    warn("Check that the file   img\\logo.png   exists next to the script file.")
    blank()

    h2("CONTACT AND SUPPORT")
    hline()
    p("If you face any issue not covered above, contact the software team.")
    p("Please note the exact error message from the Activity Log or popup.")
    p("This helps the developer fix the issue much faster.")
    blank()
    p("  Application : IndoTech Hardware Automation Platform  v1.3 (2026)")
    p("  Document    : F/DSN/54")
    p("  Developed for Indo Tech Transformers Ltd")
    p("             — Design to Production Workflow Automation")
    p("  Developer    : KRISHNA RAJU, Indo Tech-Intern")
    blank()
    hline()
